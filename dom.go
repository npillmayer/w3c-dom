...
import (
	...
	"github.com/npillmayer/w3c-dom/style"
	"github.com/npillmayer/w3c-dom/style/css"
	...
	"github.com/npillmayer/w3c-dom/w3cdom"
...
func domify(tn *tree.Node[*styledtree.StyNode]) *W3CNode {
	if sn := styledtree.Node(tn); sn != nil {
		return &W3CNode{sn}
	}
	return nil
}

// TreeNodeFromNode returns the underlying tree.Node from a DOM node.
func TreeNodeFromNode(domnode w3cdom.Node) (*tree.Node[*styledtree.StyNode], bool) {
	if domnode == nil {
		return nil, false
	}
...
// IsRoot returns wether this node is the root node of the styled tree, i.e., if IsRoot
// has no parent.
...
func (w *W3CNode) HasChildNodes() bool {
	...
	if tn, ok := TreeNodeFromNode(w); ok {
